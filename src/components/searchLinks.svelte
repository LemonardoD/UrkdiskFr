...
{#if pcd.length && configs[0].diameter.length}
	<div class="configsGrid linksGrid">
		{#each configs as rim}
			<a
				href={`${url}/rim?diameter=${rim.diameter}&width=${rim.width}&pcd=${pcd}`}
				class="link"
			>
...
{:else if rims.length && rims[0].rimId.length}
	<div class="linksGrid">
		{#each rims as rim}
			<a
				href={`/rim?id=${rim.rimId}&diameter=${rim.config[0].diameter}&width=${rim.config[0].width}&pcd=${rim.config[0].boltPattern}&carBrand=${carBrand}&carModel=${carModel}&carYear=${carYear}`}
				class="link"
			>
				{rim.brand + " - " + rim.name}
			</a>
