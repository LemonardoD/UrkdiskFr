<script lang="ts">
	...
	let hrefString =
		carBrand.length > 0
			? `/rim?id=${rimId}&diameter=${config[0].diameter}&width=${config[0].width}&pcd=${config[0].boltPattern}&carBrand=${carBrand}&carModel=${carModel}&carYear=${carYear}`
			: `/rim?id=${rimId}&diameter=${config[0].diameter}&width=${config[0].width}&pcd=${config[0].boltPattern}`;
